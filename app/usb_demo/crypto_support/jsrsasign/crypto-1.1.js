...
KJUR.crypto.MessageDigest = function(params) {
	...
	if (':md5:sha1:sha224:sha256:sha384:sha512:ripemd160:'.indexOf(alg) != -1 &&
		...
		// this.md = eval(KJUR.crypto.Util.CRYPTOJSMESSAGEDIGESTNAME[alg]).create();
		   if ("sha1" == alg) {
		    this.md = CryptoJS.algo.SHA1.create(); // TODO: we could do a case statement check here instead of eval.
	     } else {
	       throw "TODO: add the support for creating other HASH providers. eval() was disabled to support chrome apps";
	     }
	    } catch (ex) {
